...
/* ── Animated SVG price chart for Regime section ── */
...
function RiskSpider({ inView }: { inView: boolean }) {
  ...
  const innerPts = inView ? risks.map((_, i) => {
    const a = (i / n) * Math.PI * 2 - Math.PI / 2
    ...
    <div className="relative select-none">
      <svg viewBox="0 0 240 220" className="w-full" style={{ height: 220 }}>
        ...
        {[0.25, 0.5, 0.75, 1].map(sc => (
          <path key={sc} d={toPath(pts(sc))} stroke="rgba(244,225,193,0.12)" strokeWidth="1" fill="none" />
        ))}
        {/* spokes */}
        {outerPts.map((p, i) => (
          <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="rgba(244,225,193,0.1)" strokeWidth="1" />
        ))}
        <path d={toPath(outerPts)} fill="rgba(244,225,193,0.04)" stroke="rgba(244,225,193,0.12)" strokeWidth="1" />
        <path d={toPath(innerPts)} fill="rgba(244,225,193,0.12)" stroke="rgba(244,225,193,0.5)" strokeWidth="1.5"
          style={{ transition: 'all 1.2s cubic-bezier(.4,0,.2,1) 0.3s' }} />
        {innerPts.map((p, i) => (
          <circle key={i} cx={p.x} cy={p.y} r="4" fill="#F4E1C1" fillOpacity="0.8"
        ...
        {outerPts.map((p, i) => {
          ...
          return (
            <text key={i} x={p.x + dx} y={p.y + dy} textAnchor="middle" dominantBaseline="middle"
              fontSize="8" fill="rgba(244,225,193,0.5)" fontFamily="Space Mono, monospace">
              {risks[i]}
            </text>
          )
        })}
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="relative mb-6">
        <div className="absolute -inset-px rounded-3xl" style={{ background: 'linear-gradient(135deg, rgba(0,128,128,0.3), rgba(0,128,128,0.06))' }} />
        <div className="relative rounded-3xl px-8 py-6" style={{ background: 'rgba(244,225,193,0.6)', backdropFilter: 'blur(20px)' }}>
...
export function CoreFeatures() {
  ...
  return (
    <div id="features">
      ...
      <section className="relative py-28 overflow-hidden bg-[#F4E1C1]" ref={f01.ref}>
        <div className="absolute top-1/2 -translate-y-1/2 -left-8 font-['Syne'] font-extrabold text-[22vw] text-[rgba(0,128,128,0.04)] leading-none select-none pointer-events-none">01</div>
        ...
        <div className="absolute top-0 right-0 w-80 h-80 opacity-20 pointer-events-none"
          style={{ backgroundImage: 'radial-gradient(circle,rgba(0,128,128,0.3) 1px,transparent 1px)', backgroundSize: '18px 18px' }} />

        <div className="max-w-7xl mx-auto px-6 xl:px-20">
          <div className="grid lg:grid-cols-2 gap-16 xl:gap-28 items-center">
            {/* copy */}
            <div className="relative z-10"
              style={{ opacity: f01.inView ? 1 : 0, transform: f01.inView ? 'translateX(0)' : 'translateX(-2rem)', transition: 'all 0.8s cubic-bezier(.4,0,.2,1)' }}>
              ...
              <h2 className="font-['Syne'] font-extrabold text-[#0d2b2b] leading-[1.0] tracking-[-0.025em] mb-6"
                style={{ fontSize: 'clamp(3rem,5vw,4.2rem)' }}>
                We check the<br />market's mood<br />
                <span className="grad-teal">before investing.</span>
              </h2>
              <p className="text-[#0d2b2b]/52 text-lg leading-[1.8] mb-8 max-w-[400px]">
                Is the market a heatwave or a snowstorm? You don't wear the same clothes in both. Our system detects the regime first — Bull, Bear, Volatile, or Sideways — then dresses your portfolio accordingly.
              </p>
              <div className="space-y-0">
                {[
                  ...
                  <div key={s.r}
                    ...
                    style={{ opacity: f01.inView ? 1 : 0, transform: f01.inView ? 'translateX(0)' : 'translateX(-12px)', transition: `all 0.5s ease ${0.3 + i * 0.1}s` }}>
              ...
              <p className="mt-6 font-mono text-xs text-[#0d2b2b]/35 italic">
                "Most tools give you the same advice no matter what's happening. That's why they fail."
              </p>
            </div>

            {/* chart */}
            <div style={{ opacity: f01.inView ? 1 : 0, transform: f01.inView ? 'translateX(0)' : 'translateX(2rem)', transition: 'all 0.8s cubic-bezier(.4,0,.2,1) 0.2s' }}>
              <div className="absolute -inset-8 bg-[radial-gradient(ellipse,rgba(0,128,128,0.08)_0%,transparent_70%)] blur-2xl pointer-events-none" style={{ position: 'relative', inset: 'auto' }} />
              <div className="relative rounded-3xl px-8 py-10"
                style={{ background: 'rgba(244,225,193,0.6)', backdropFilter: 'blur(24px)', border: '1px solid rgba(0,128,128,0.14)', boxShadow: '0 8px 48px rgba(0,128,128,0.08), inset 0 1px 0 rgba(255,255,255,0.6)' }}>
                ...
                <div className="mt-4 flex items-center justify-between">
                  ...
                  <div className="px-3 py-1.5 rounded-full bg-[rgba(0,128,128,0.1)] border border-[rgba(0,128,128,0.2)]">
      ...
      <section className="relative py-28 overflow-hidden" ref={f02.ref}
        ...
        <div className="absolute top-0 left-0 right-0 h-20 pointer-events-none"
          style={{ background: 'linear-gradient(180deg, rgba(244,225,193,0.07) 0%, transparent 100%)' }} />
        {/* bottom accent line + fade */}
        <div className="absolute bottom-0 left-0 right-0 h-px pointer-events-none"
          style={{ background: 'linear-gradient(90deg, transparent, rgba(0,128,128,0.35), transparent)' }} />
        <div className="absolute bottom-0 left-0 right-0 h-20 pointer-events-none"
          style={{ background: 'linear-gradient(0deg, rgba(244,225,193,0.07) 0%, transparent 100%)' }} />
        <div className="absolute top-1/2 -translate-y-1/2 -right-8 font-['Syne'] font-extrabold text-[22vw] text-[rgba(244,225,193,0.025)] leading-none select-none pointer-events-none">02</div>
        <div className="absolute top-1/2 left-1/4 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] rounded-full border border-[rgba(244,225,193,0.04)] animate-spin-very-slow pointer-events-none" />
        <div className="absolute top-0 left-0 w-80 h-full opacity-10 pointer-events-none"
          style={{ backgroundImage: 'radial-gradient(circle,rgba(244,225,193,0.3) 1px,transparent 1px)', backgroundSize: '22px 22px' }} />

        <div className="max-w-7xl mx-auto px-6 xl:px-20">
          <div className="grid lg:grid-cols-2 gap-16 xl:gap-28 items-center">
            {/* spider — left */}
            <div style={{ opacity: f02.inView ? 1 : 0, transform: f02.inView ? 'translateX(0)' : 'translateX(-2rem)', transition: 'all 0.8s cubic-bezier(.4,0,.2,1)' }}>
              ...
              <div className="mt-6 flex flex-wrap gap-2 justify-center">
                {[
                  ...
                  <span key={r.l}
                    ...
                    style={{ color: r.c, background: `${r.c}18`, border: `1px solid ${r.c}30`, opacity: f02.inView ? 1 : 0, transition: `opacity 0.4s ease ${0.8 + i * 0.15}s` }}>
                    {r.l} · {r.s}
                  </span>
                ))}
              </div>
            ...
            {/* copy — right */}
            <div style={{ opacity: f02.inView ? 1 : 0, transform: f02.inView ? 'translateX(0)' : 'translateX(2rem)', transition: 'all 0.8s cubic-bezier(.4,0,.2,1) 0.2s' }}>
              <div className="flex items-center gap-3 mb-6">
                <span className="font-mono text-[9px] tracking-[0.28em] uppercase text-[rgba(244,225,193,0.25)]">02 /</span>
                <span className="font-mono text-[9px] tracking-[0.18em] uppercase text-[#c49a60]">The Safeguard</span>
              </div>
              <h2 className="font-['Syne'] font-extrabold text-[#F4E1C1] leading-[1.0] tracking-[-0.025em] mb-6"
                style={{ fontSize: 'clamp(3rem,5vw,4.2rem)' }}>
                Our AI argues<br />with itself<br />
                <span className="text-[#c49a60]">to protect you.</span>
              </h2>
              <p className="text-[rgba(244,225,193,0.5)] text-lg leading-[1.8] mb-10 max-w-[400px]">
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={r.title}
                    className="flex gap-4 py-4 border-b border-[rgba(244,225,193,0.06)]"
                    style={{ opacity: f02.inView ? 1 : 0, transform: f02.inView ? 'translateX(0)' : 'translateX(12px)', transition: `all 0.5s ease ${0.5 + i * 0.15}s` }}>
                    <span className="text-lg flex-shrink-0 mt-0.5">{r.icon}</span>
                    <div>
                      <div className="font-['Syne'] font-semibold text-sm text-[#F4E1C1] mb-0.5">{r.title}</div>
                      <div className="font-mono text-[10px] text-[rgba(244,225,193,0.35)]">{r.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
              <p className="mt-8 font-mono text-xs text-[rgba(244,225,193,0.25)] italic">
      ...
      <section className="relative py-28 overflow-hidden bg-[#F4E1C1]" ref={f03.ref}>
        ...
        <div className="max-w-7xl mx-auto px-6 xl:px-20">
          <div className="text-center mb-16"
            style={{ opacity: f03.inView ? 1 : 0, transform: f03.inView ? 'translateY(0)' : 'translateY(2rem)', transition: 'all 0.7s cubic-bezier(.4,0,.2,1)' }}>
            ...
            <p className="text-[#0d2b2b]/52 text-lg leading-[1.78] max-w-xl mx-auto">
          ...
          <div style={{ opacity: f03.inView ? 1 : 0, transform: f03.inView ? 'translateY(0)' : 'translateY(2rem)', transition: 'all 0.7s cubic-bezier(.4,0,.2,1) 0.2s' }}>
          ...
          <div className="mt-16 flex flex-wrap justify-center gap-3">
            {[
              ...
              <span key={ex}
                ...
                style={{ background: 'rgba(0,128,128,0.04)', opacity: f03.inView ? 1 : 0, transition: `all 0.4s ease ${0.6 + i * 0.1}s` }}>
                {ex}
              </span>
            ))}
          </div>
